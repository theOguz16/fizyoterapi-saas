...
export function enrichPackageRowForDisplay(pkg: Package, catalog: LessonCatalogItem[]): AdminPackage {
  ...
  return {
    ...
    weekly_class_hours: Math.min(7, Math.max(1, Math.floor(Number(rules.weekly_class_hours ?? 1) || 1))),
    required_preference_slots: Math.min(7, Math.max(1, Math.floor(Number(rules.weekly_class_hours ?? 1) || 1))) * 3,
    required_trainer_free_slots: Math.min(7, Math.max(1, Math.floor(Number(rules.weekly_class_hours ?? 1) || 1))) * 2,
