...
export default function TrainerBulkNotificationScreen() {
  ...
  return (
    <AppShell title="Toplu bildirim" subtitle="Seçili danışanlara şablon kullanarak mesaj gönder ve teslim sonucunu gör." icon="notifications" showBackButton refreshing={membersQuery.isRefetching} onRefresh={() => void membersQuery.refetch()}>
      <SurfaceCard>
        <Text style={styles.section}>Mesaj şablonu</Text>
        <View style={styles.chips}>{TEMPLATES.map((template) => <SelectionChip key={template.label} label={template.label} active={title === template.title} onPress={() => { setTitle(template.title); setBody(template.body); }} />)}</View>
        <FormField label="Başlık" value={title} onChangeText={setTitle} placeholder="Bildirim başlığı" />
        <FormField label="Mesaj" value={body} onChangeText={setBody} placeholder="Mesajını yaz" multiline numberOfLines={4} />
      </SurfaceCard>
      <SurfaceCard>
        ...
        <View style={styles.chips}>{members.map((member: any) => <SelectionChip key={member.id} label={member.full_name || `${member.first_name || ""} ${member.last_name || ""}`.trim() || "Üye"} active={selected.includes(String(member.id))} onPress={() => toggle(String(member.id))} />)}</View>
        <ActionButton label="Seçili üyelere gönder" icon="notifications" loading={mutation.isPending} disabled={!selected.length || !body.trim()} onPress={() => mutation.mutate()} />
      </SurfaceCard>
    </AppShell>
  );
}
